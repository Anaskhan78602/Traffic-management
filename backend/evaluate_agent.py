...
def main():
    ...
    if os.path.exists("models/trained_q_table.pkl") and agent.load_model("models/trained_q_table.pkl"):
        ...
        agent = train_agent(episodes=50, steps_per_episode=300, render_plot=False)
    ...
    if fixed_avg == 0 or ai_avg == 0:
      improvement = 0
    elif not np.isfinite(fixed_avg) or not np.isfinite(ai_avg):
       improvement = 0
    else:
